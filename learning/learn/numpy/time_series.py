    ...
    with tf.Session() as sess:
        ...
        for step in range(10000):
            ...
            if step % 10 == 0:
                # 用测试数据评估loss
                print(step, loss_)
        print("保存模型: ", saver.save(sess, 'E:/python_project/model/model.ckpt'))
...
def prediction():
    ...
    with tf.Session() as sess:
        # tf.get_variable_scope().reuse_variables()
        saver.restore(sess, 'E:/python_project/model/model.ckpt')
...
data_set = pd.read_csv("E:/python_project/data/railway_flow.csv")
x_data = np.array(data_set['时间'])
data = np.array(data_set['铁路客运量_当期值(万人)'])
# normalize
